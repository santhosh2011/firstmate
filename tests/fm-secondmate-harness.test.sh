...
#      directories inherit the primary's settings. For config/herdr-presentation-spaces,
#      an absent primary file and an absent destination file both mean the same
...
/second/limit' ] || fail "changed no-go-paths did not converge"
  ...
  rm -f "$src/crew-dispatch.json" "$src/crew-harness" "$src/backlog-backend" \
    "$src/backend" "$src/herdr-presentation-spaces" "$src/no-go-paths" "$src/trace-context"
  propagate_inheritable_config "$src" "$dest"
