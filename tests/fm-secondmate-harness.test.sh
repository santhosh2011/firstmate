...
#      config/startup-memory-budget, config/trace-context, and config/no-go-paths -
...
#      presentation choice, startup-memory budget, trace context, and off-limits
#      directories inherit the
...
/second/limit' ] || fail "changed no-go-paths did not converge"
  ...
  rm -f "$src/crew-dispatch.json" "$src/crew-harness" "$src/backlog-backend" \
    "$src/backend" "$src/herdr-presentation-spaces" "$src/trace-context" "$src/no-go-paths"
  propagate_inheritable_config "$src" "$dest"
...
test_propagate_unusable_source_is_an_error_not_absence() {
  ...
  for label in directory dangling-symlink symlink-to-directory unreadable-file; do
    ...
    assert_contains "$(cat "$stderr")" "fm-config-inherit: error:" \
      "$label: no stderr diagnostic reported the unusable source"
    assert_contains "$(cat "$stderr")" "no-go-paths" \
      "$label: the diagnostic did not name the item"
...
test_propagate_undeterminable_source_dir_never_mirrors_absence() {
  ...
  assert_contains "$(cat "$stderr")" 'cannot inspect primary source' \
    "undeterminable source: no diagnostic named the failed lookup"
  assert_contains "$(cat "$stderr")" 'Permission denied' \
