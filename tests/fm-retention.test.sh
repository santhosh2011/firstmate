...
test_orphan_treehouse_worktree_uses_its_own_evidence() {
  ...
  # A pooled clone carries its base as a remote-tracking ref. Retention never
  # fetches, so that ref is the only landed-work proof it may use; without it
  # the worktree is correctly retained rather than swept.
