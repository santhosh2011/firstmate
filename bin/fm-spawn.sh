...
#   That refresh is for the reused treehouse pool slot; an SDev workspace is
#   excluded because `sdev new` already fetches each repo's base from origin.
...
# A relaunch reuses the workspace recorded in the task's own meta (WT is already
# set from it above), so it must never run `sdev new` against a slug that exists.
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # An SDev task's recorded worktree is the multi-repo workspace directory, not a
  # single git worktree root, so it proves isolation per repo instead.
  if [ "$KIND" != secondmate ]; then
    if [ "$IS_SDEV" = 1 ]; then
      validate_spawn_sdev_workspace
    else
...
# This refresh exists for the reused treehouse pool slot, whose checkout persists
# between tasks and can be stale. An SDev workspace is excluded, and redundantly
# so rather than merely inapplicable: `sdev new` fetches origin/<base> for each
# repo and starts task/<slug> from the freshest base, including when it re-brands
# a pooled worktree (sdev/bin/new-task). Do not "fix" this by dropping the
# IS_SDEV test - $WT is the multi-repo workspace directory, not a git repository,
# so the fetch below would fail and refuse every SDev spawn.
