...
That is this task's delivery contract as a whole. Firstmate resolves each repo's own landing mode from the landing overlay when it coordinates the ship, so a single repo may land differently; nothing about that is yours to decide.
...
EOF
echo "scaffolded: $BRIEF (ship, sdev multi-repo, mode=$MODE; replace {TASK})"
exit 0
fi
