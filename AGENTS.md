...
config/no-go-paths  optional absolute directory prefixes that agent work must never be dispatched into; LOCAL, gitignored; absent means unrestricted, a match refuses the spawn naming the path and the prefix; inherited by secondmate homes so one cannot route around it; see docs/configuration.md "No-go paths"
