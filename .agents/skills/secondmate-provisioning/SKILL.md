...
`config/no-go-paths` is the one exception, because it is a safety boundary rather than an advisory default: when the primary has one and it could not be written into the home, `fm-spawn.sh --secondmate` refuses the launch instead of starting a home whose crewmates would see no restriction.
...
A primary item that exists but is not a readable regular file, such as a dangling symlink into a reorganised dotfiles tree, is a propagation error and never absence, so the destination copy is left untouched rather than removed.
